...
**Hecha y cerrada (2026-09-19, decision de Mariano: "usar el sistema").** 4a-4h entregadas
con tests (64). `pesos.py` lleva la convencion del factorial como parametro y todo indice
se calcula sobre la convencion fuera. A partir de aqui **no se construye mas**: solo
mantenimiento de lo que falle en el uso real. Polisectores, cuando la tesis lo pida.
...
| 2026-09-19 | 1 (corpus) | Decisiones de Mariano: marca `texto: arXiv vN` sin excepcion; sin tercera pasada. `ingest --arxiv ID [nombre.pdf] --etiquetas --factorial` baja el PDF y el e-print; tres papers nuevos por tex y verificados: `lastra-sanz-2010` (AIF 60, `citado`), `sanz-2014` (JMAA 415, `citado`), `thilliez-2008` (Expo. Math. 26, `directores`; paginas del PDF de revista de Mariano `thilliez-2006.pdf`, texto del e-print v5 via `--arxiv ID pdf_existente`). Campo `factorial` (dentro/fuera/no_aplica/pendiente) en los 25 documentos, comprobado en la definicion de la clase de cada uno (`registro/verificaciones.md`); `ask` lo muestra en DOCUMENTOS y en la linea de notacion; `etiquetas` y `factorial` sobreviven a `--rehacer`; `renombrar` actualiza `meta.json` (11 ids viejos corregidos). Prop. 4.13 de LMS 2015: p+1 en arXiv v1 y en Sanz 2014; la revista no esta en el corpus. 4.284 fragmentos; 61 tests | De Mariano: `factorial` de JGSS 2017 y JGCSS 2026 stability (solo asintotica); `## Mi explicacion` + `nota check`; direccion. Agente: nada mas (BRIEF 7) |
| 2026-09-19 | 4 (cierre), 1 | Decisiones de Mariano: `factorial` de JGSS 2017 y JGCSS 2026 stability = fuera por su regla (analitica <=> M = 1 es fuera; <=> M_p = p! es dentro); LMS 2015 se queda con arXiv v1 y la fila (c) de la hoja de aceptacion queda correcta respecto a su fuente (revista: p-1, mismo limite); direccion: USAR EL SISTEMA. `pesos.py` con la convencion como parametro (`factorial`, `en()`), `gevrey(alpha, factorial)`, `indice_omega` siempre sobre la fuera [sanz-2014:p9, Teorema 3.2], (gamma_1) de Thilliez sobre la fuera, `panel` con omega(M); test: la misma Gevrey en las dos convenciones da el mismo omega. 64 tests. **Construccion cerrada: solo mantenimiento.** | De Mariano: `## Mi explicacion` + `nota check`; `gamma(M)` en pesos.py (suyo). Agente: nada, salvo lo que falle en el uso |
